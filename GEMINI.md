...
2. **Discovery Phase (No Directive Found)**: If no directive exists, you MUST NOT proceed to implementation. Instead:
    - **CRITICAL**: You MUST ALWAYS enter a questioning phase first. No exceptions.
    - Ask clarifying questions until you can define the goals, inputs, tools, and outputs (similar to the directive template).
    - Once the scope is clear, you MUST ask: **"Should I create a new directive for this, skip directly to the execution layer (whatever is left), or should I stay in discovery?"**
    - Do NOT move to implementation until the user explicitly chooses the execution path.
